...
public class ComandosSqlSingleton {
	...
	private final String SELECT_TAREFA_LEMBRETE_POR_USUARIO = "SELECT DISTINCT T.ID, T.TITULO, T.DESCRICAO,T.HORA, U.NOME " 
							+"FROM TAREFA T, USUARIO U " 
							+"WHERE T.USUARIO_ID = ? " 
							+"AND HORA IS NOT NULL AND PROGRESSO IS NULL";																
					
	private final String SELECT_TAREFA_PROGRESSIVA_POR_USUARIO = "SELECT DISTINCT T.ID, T.TITULO, T.DESCRICAO,T.PROGRESSO, U.NOME "
							+ "FROM TAREFA T, USUARIO U "
							+ "WHERE T.USUARIO_ID = ? AND HORA IS NULL "
